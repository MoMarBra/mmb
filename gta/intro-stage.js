...
export class IntroStage {
  ...
  light(t) {
    ...
    if (!s) return;
    if (s.prologue) {
      w.renderer.toneMappingExposure = 0.62;
      w.scene.environmentIntensity = 0.24;
      w.ambient.color.set('#9ab7d5');
      w.ambient.groundColor.set('#37495e');
      w.fill.color.set('#8daac8');
      w.ambient.intensity = 0.3;
      w.fill.intensity = 0.2;
      w.sun.intensity = 0.07;
      w.scene.background.set('#102035');
      const sky = this.g.arcade.atmosphere.sky.material.uniforms;
      sky.top.value.set('#071423');
      sky.bottom.value.set('#152a42');
